...
// 선택된 정렬 순서를 강조
document.querySelectorAll("[name='sortNoLi'] > a")[sortNo].classList.add("sortNoSelected");
...
if(categoryCode == 1) {    
    ...
    // 대여 지역 선택값이 바뀌면 요청
    document.getElementById("carLocationNoSelect").addEventListener("change", () => {getItemList()});

    // 대여 / 반납 날짜가 바뀌면 요청
    const rentExpectDateInput = document.querySelectorAll("#rentDate, #expectDate");
    ...
    // 검색 버튼이 눌리면 요청
    document.getElementById("serachBtn").addEventListener("click", () => {getItemList()});
