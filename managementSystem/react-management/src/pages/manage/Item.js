...
const Item = () => {
  ...
  const changeCategoryCode = (e) => {
    ...
    setColumn(
      e.target.value == '1' ? carColumn : (
        e.target.value == '2' ? campEquipmentColumn : packageColumn
      )
    );
  ...
  const changeData = (categoryCode) => {
    axios.get(`/manage/item?categoryCode=${categoryCode}`)
    .then(res => {
        setData(res.data.itemList);
  ...
  const table = useMaterialReactTable({
    ...
    renderTopToolbarCustomActions: ({ table }) => (
      <>
        {['차량 관리', '캠핑용품 관리', '패키지 관리'].map((text, index) => {
          return(
            <Button key={index} variant="contained" value={index + 1} onClick={changeCategoryCode}
            >{text}
            </Button>
          );
        })}
    ...
    renderDetailPanel: ({ row }) => (
      <ItemDetail key={row.id} itemNo={row.original.itemNo} categoryCode={categoryCode} />
    ),
  });
